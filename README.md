...
- Assumes newvers.sh creates sha512 and sha256 files for all components of the link kit, including makegap.sh and lorder and all .o files, and a separate file for the linked kernel from the build
Precondition 2:
...
IF obj.{id}.sha512 AND kernel.{id}.sha512 AND obj.{id}.sha256 AND kernel.{id}.sha256, when checked using sha512 -C and sha256 -C match checksum files stored in /var/db AND that there are no missing files
   {reorder_kernel}
ELSE
  {log error}
  {exit 1}

Either a valid kernel will be relinked or an error will be logged and an exit status of 1 will occur in /etc/rc
