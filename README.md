...
Every time a new version[a] of Unix is created, it needs to be stamped as authentic, otherwise it's just a glorified computer worm susceptible to attack as indicated by Paul Karger[1] masquerading as an operating system that people are trying to do genetic engineering (or hopelessly, analysis) on, as iterated builds and porting to different host architectures make it _self-reproducing_. Discussion must subsume the self-reproductive nature of  components in complicated software builds as an engineering problem concerning the integrity of _build processes_. This needs to be treated as a systems engineering, computer security, supply chain, and technical computing problem simultaneously, and provided an interdisciplinary treatment. The Unix kernel (as traditionally implemented) defines the entire security domain of a running system[3], so the build process for such a kernel needs to have integrity guarantees at a different level of abstraction, and specific security controls to prevent tampering. The creation of such a stamp avoids a circular specification of the operating system where it is defined as its implementation (within the security domain of the operating system's kernel) and an endless search for a bootstrapping procedure - it can instead be considered a reference copy of a component with a complicated and opaque internal structure the modularity of which is known only to the chain of authors, or those possessing a complicated graph of its internal structure and subcomponents as well as specifications of all interfaces, dependencies, and _inclusions_.

The problem of inversion from the ELF machine code back to the C source code is in general undecidable as the Post correspondence problem is very well-understood [4], so data integrity of the objects needs to be accounted for by *some* mechanism. Furthermore, the existing C compiler also lacks type information inside its intermediate representation, and payloads _malicious or unwanted inclusions_ (of course) may be arbitrarily encoded inside or outside the kernel, which, as already stated, defines the entire security domain on a self-hosted system. Kernel-reordering as currently implemented in OpenBSD exposes the kernel to complete replacement as the reordering script does not check that the objects which are being recompiled are those which were originally built to link the running kernel, and in fact allows relinking of arbitrary objects, which then replace the image on disk and are booted during the next boot cycle.

SLSA provenance (https://slsa.dev/spec/v0.1/levels) provides a framework to directly discuss the shortcomings in the build system for the official releases in lieu of a clean-room implementation of a build environment ("SLSA4"), but at present OpenBSD is still self-hosting and self-bootstrapping from a machine code kernel of unknown provenance. Before kernel reordering was introduced it had a tamper-resistant build process ("SLSA2"), with a clean build always proceeding from source and thus was at a higher level of SLSA compliance than it is now: an automatic process relinks kernel objects of unknown provenance at install time and at boot time (a "link kit"), and does not match them against any checksums or digital signatures at either the host level or externally. My proposal to add specific security controls to the reorder_kernel function for exactly this reason (hardening the build process) was rejected, as of writing, perhaps for lack of motivation or context or differing concerns. 

Furthermore, in the past, GCC support was also present so bootstrapping a build offline from another environment that does not suffer from arbitary relinking present in the current self-hosted environment ("hermetic builds" in SLSA parlance) was less difficult. The vulnerability can be avoided by making the kernel-reordering function transactional, or by decoupling the components used for reordering from the automatic startup script and into a daemon which is activated or deactivated at the discretion of the user (selectable at install-time before any reordering has occured). C code was originally intended to be highly portable and the POSIX specification was developed to facilitate this, however the OpenBSD code as currently implemented is tightly coupled to an archaic and mostly-undocumented build system, and not portable to even the packaged GCC compiler using the currently-available source releases. Historical review (of potentially unavailable or doctored) historical releases of OpenBSD may be practical.

This essay limits the topic to auditability and security-hardening (tamper resistance) of the build process and not higher-level theoretical limitations and practical problems in compiler design and formal verification. It essentially argues that the Multics security evaluation now applies to OpenBSD due to its peculiar, perhaps unintentional similarity concerning the shortcomings of storing kernel objects (as individual components) directly in the filesystem, without considering system integrity as a whole when rebuilding them. A situation similar to the spread of Stuxnet is possible because the install images do not embed signatures and checksums for the install sets within the media, with the complete image also being signed and checksummed, leaving it open to tampering. Considering all the components of the system at several levels of abstraction is important. 

Keywords: Unix, OpenBSD, build process engineering, security evaluation, system integrity, SLSA

[a] ("release" in OpenBSD parlance)
...
[4] https://www.cis.upenn.edu/~jean/gbooks/PCPh04.pdf
