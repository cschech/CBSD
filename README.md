...
Every time a new version ("release" in OpenBSD parlance) of Unix is created, it needs to be stamped as authentic, otherwise it's just a glorified computer worm susceptible to attack as indicated by Paul Karger (NB: the spurious topic "trust-of-trust" of Dennis Richie's popular ACM lecture was a deliberate misconstrual of the USAF security analysis and compromise of Multics[1]) masquerading as an operating system that people are trying to do genetic engineering (or perhaps hopelessly, analysis) on, as iterated builds make it _self-reproducing_. Discussion must subsume the self-reproductive nature of  components in complicated software build processes as given, and discussion needs to shift away from theoretical properties of compilers toward engineering problems concerning _build processes_. This needs to be treated as a systems engineering, computer security, supply chain, and technical computing problem simultaneously, and provided an interdisciplinary treatment. The Unix kernel (as traditionally implemented) defines the entire security domain of a running system[3]. The creation of such a stamp avoids a circular specification of the operating system where it is defined as its implementation, and an endless search for a bootstrapping procedure - it can instead be considered a reference copy of a component with a complicated and opaque internal structure the modularity of which is known only to the chain of authors, or those possessing a complicated graph of its internal structure and subcomponents as well as specifications of all interfaces, dependencies, and _inclusions_.
