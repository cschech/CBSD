...
Every time a new version ("release" in OpenBSD parlance) of Unix is created, it needs to be stamped as authentic, otherwise it's just a glorified computer worm susceptible to attack as indicated by Paul Karger (NB: the spurious topic "trust-of-trust" of Dennis Richie's popular ACM lecture was a deliberate misconstrual of the USAF security analysis and compromise of Multics[1]) masquerading as an operating system that people are trying to do genetic engineering (or perhaps hopelessly, analysis) on, as iterated builds make it _self-reproducing_. To avoid digressions into "trust of trust" this needs to be treated as a systems engineering, computer security, supply chain problem, and a technical computing problem simultaneously, and given an interdisciplinary treatment, as the Unix kernel (as traditionally implemented) defines the entire security domain of a running system[3].
