...
SLSA provenance (https://slsa.dev/spec/v0.1/levels) would be a path toward (partial) assurance for official releases in lieu of a clean-room implementation of a build environment ("SLSA4"), but at present OpenBSD is still self-hosting and self-bootstrapping from a machine code kernel of unknown provenance. Before kernel reordering was introduced it had a tamper-resistant build process ("SLSA2") and thus was at a higher level of SLSA compliance than it is now, where an automatic process relinks kernel objects of unknown provenance at install time and at boot time. In the past, GCC support was also present so bootstrapping a build from another environment was less difficult. My proposal to add specific security controls to the reorder_kernel function for exactly this reason was rejected, as of writing.
...
IF obj.{id}.sha512 AND kernel.{id}.sha512 AND obj.{id}.sha256 AND kernel.{id}.sha256, and kernel.SHA256 (the existing hash kernel_reorder *does* check which is generated by the make program) when checked using sha512 -C and sha256 -C match checksum files stored in /var/db AND that there are no missing files
THEN   
   <reorder_kernel>
ELSE
  <log error>
