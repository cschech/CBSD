...
Either a valid kernel will be relinked or an error will be logged and an exit status of 1 will occur in /etc/rc, both newvers.sh
There are a lot of interacting components so a thorough implementation will take this into account. Also builds can be switched between, which isn't a concern of kernel_reorder but object and kernel integrity is.
